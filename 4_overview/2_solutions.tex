их\section{Существующие решения.}
...
\begin{gather*}
  ...
  = \begin{bmatrix}
    \sum \limits_{i = 1}^n \tilde{s}_{ix} \cdot \tilde{k}_{s_i x}^T & \sum \limits_{i = 1}^n \tilde{s}_{ix} \cdot \tilde{k}_{s_i y}^T & \sum \limits_{i = 1}^n \tilde{s}_{ix} \cdot \tilde{k}_{s_i z}^T \\
    \sum \limits_{i = 1}^n \tilde{s}_{iy} \cdot \tilde{k}_{s_i x}^T & \sum \limits_{i = 1}^n \tilde{s}_{iy} \cdot \tilde{k}_{s_i y}^T & \sum \limits_{i = 1}^n \tilde{s}_{iy} \cdot \tilde{k}_{s_i z}^T \\
    \sum \limits_{i = 1}^n \tilde{s}_{iz} \cdot \tilde{k}_{s_i x}^T & \sum \limits_{i = 1}^n \tilde{s}_{iz} \cdot \tilde{k}_{s_i y}^T & \sum \limits_{i = 1}^n \tilde{s}_{iz} \cdot \tilde{k}_{s_i z}^T
  \end{bmatrix}.
\end{gather*}
...
Ранг матрицы $ \Sigma $ меньше трёх, если \cite{rankAB}
\begin{equation*}
  rank \, X =
...
\end{equation*}
Такое возможно,
если $ \det{X} = \det{ \left( \tilde{S} \cdot \tilde{K}^T \right) } = 0$ \cite{faddeev:rank:det}.

Раскроем этот определитель
\begin{gather*}
  \det{X} = \\
  = \sum \limits_{i = 1}^n \tilde{s}_{ix} \cdot \tilde{k}_{s_i x}^T \cdot \left(
    \sum \limits_{i = 1}^n \tilde{s}_{iy} \cdot \tilde{k}_{s_i y}^T \cdot
    \sum \limits_{i = 1}^n \tilde{s}_{iz} \cdot \tilde{k}_{s_i z}^T -
    \sum \limits_{i = 1}^n \tilde{s}_{iz} \cdot \tilde{k}_{s_i y}^T \cdot
    \sum \limits_{i = 1}^n \tilde{s}_{iy} \cdot \tilde{k}_{s_i z}^T \right) - \\
  - \sum \limits_{i = 1}^n \tilde{s}_{iy} \cdot \tilde{k}_{s_i x}^T \cdot \left(
    \sum \limits_{i = 1}^n \tilde{s}_{ix} \cdot \tilde{k}_{s_i y}^T \cdot
    \sum \limits_{i = 1}^n \tilde{s}_{iz} \cdot \tilde{k}_{s_i z}^T -
    \sum \limits_{i = 1}^n \tilde{s}_{iz} \cdot \tilde{k}_{s_i y}^T \cdot
    \sum \limits_{i = 1}^n \tilde{s}_{ix} \cdot \tilde{k}_{s_i z}^T \right) + \\
  + \sum \limits_{i = 1}^n \tilde{s}_{iz} \cdot \tilde{k}_{s_i x}^T \cdot \left(
    \sum \limits_{i = 1}^n \tilde{s}_{ix} \cdot \tilde{k}_{s_i y}^T \cdot
    \sum \limits_{i = 1}^n \tilde{s}_{ix} \cdot \tilde{k}_{s_i z}^T -
    \sum \limits_{i = 1}^n \tilde{s}_{ix} \cdot \tilde{k}_{s_i y}^T \cdot
    \sum \limits_{i = 1}^n \tilde{s}_{iy} \cdot \tilde{k}_{s_i z}^T \right) = \\
  = \prod \left(
    \tilde{k}_{s_1 x}^T, \tilde{k}_{s_1 y}^T, \tilde{k}_{s_1 z}^T, \dotsc,
    \tilde{k}_{s_n x}^T, \tilde{k}_{s_n y}^T, \tilde{k}_{s_n z}^T
  \right),
\end{gather*}
где $ \prod $~---~многочлен.
Покажем, что вероятность того, что определитель равен нулю, равна нулю.
Выпишем отдельно одну переменную многочлена $ \prod $
\begin{gather*}
  \tilde{k}_{s_1 x}^T =
  \left[
    R \cdot \vec{s}_1 + \vec{b} + \vec{ \xi }_{s_1} -
    \frac{1}{ \left| S \right| } \sum \limits_{s \in S}
      \left( R \cdot \vec{s} + \vec{b} + \vec{ \xi }_s \right) \right]_x^T = \\
  = \left[
    R \cdot \vec{s}_1 + \vec{b} - \frac{1}{ \left| S \right| } \sum \limits_{s \in S}
      \left( R \cdot \vec{s} + \vec{b} \right) \right]_x^T +
  \left[
    \vec{ \xi }_{s_1} -
    \frac{1}{ \left| S \right| } \sum \limits_{s \in S} \vec{ \xi }_{s} \right]_x^T \sim \\
  \sim N \left(
    \left[ R \cdot \vec{s}_1 + \vec{b} -
    \frac{1}{ \left| S \right| } \sum \limits_{s \in S} \left(
      R \cdot \vec{s} + \vec{b}
    \right) \right]_x^T,
    \frac{ \left| S \right| - 1}{ \left| S \right| } \cdot \sigma^2
  \right).
\end{gather*}
...
\begin{gather*}
  ...
  M P \left(
    \det{X} = 0 \; \middle|
    \tilde{k}_{s_1 y}^T, \tilde{k}_{s_1 z}^T, \dotsc,
    \tilde{k}_{s_n x}^T, \tilde{k}_{s_n y}^T, \tilde{k}_{s_n z}^T
  \right) = \\
  = M \mathbbm{1} \left\{
    F_{ \tilde{k}_{s_1 y}^T, \tilde{k}_{s_1 z}^T, \dotsc, \tilde{k}_{s_n x}^T, \tilde{k}_{s_n y}^T, \tilde{k}_{s_n z}}
    \left( \tilde{k}_{s_1 x} \right) = 0
  \right\},
\end{gather*}
где $F_{a, b, c, \dotsc, d} \left( \cdot \right) $~---~многочлен.
Фиксируем элементы, которые стоят в его индексе.
Тогда
\begin{equation*}
  P \left\{ F_{a, b, c, \dotsc, d} \left( \tilde{k}_{s_1 x} \right) = 0 \right\} = 0.
\end{equation*}
Так как $\tilde{k}_{s_1 x}$~---~случайная величина с непрерывным распределением,
то она принимает фиксированное значение с нулевой вероятностью.
Таким образом, $P \left( \det{X} = 0 \right) = M 0 = 0$,
и матрица $ \Sigma $ будет иметь ранг 3 с вероятностью 1,
то есть имеет место неравенство $ \sigma_1 \geq \sigma_2 \geq \sigma_3 > 0$.
Это означает, что сингулярное разложение матрицы будет единственным.
...
Покажем, что итеративный алгоритм ближайших точек всегда сходится.
Предположим, что алгоритм никуда не сходится.
Тогда существует такое конечное множество $S$, матрица поворота $R$,
вектор смещения $ \vec{b}$ и шум $ \vec{ \xi}_s$,
что алгоритм будет выполнять бесконечно много итераций и никогда не остановится.

Пусть алгоритм перебрал все возможные разметки,
то есть выполнил $ \left| S \right|^{ \left| S \right| }$ итераций, и не остановился.
В этом случае на следующем шаге будет выбрана разметка,
которая уже выбиралась на одном из предыдущих шагов.
Тогда выражение либо \label{eq:least:squares} увеличится, либо не изменится.
Первый случай невозможен.
Во втором случае алгоритм завершит свою работу.
Это означает, что алгоритм всегда сходится.
