...
\end{equation*}
Параметры, которые входят в каждое слагаемое, независимы, так что
\begin{align*}
...
Пусть $R$~---~фиксирована, обозначим
\begin{equation*}
  E \left( \vec{b} \right) =
  \sum \limits_{s \in S} \left \Vert R \cdot \vec{s} + \vec{b} - \vec{k}_s \right \Vert^2.
\end{equation*}
Можем найти оптимальное смещение, взяв производную $E$ по $ \vec{b}$ и приравняв к нулю
...
\begin{gather*}
  ...
  = \sum \limits_{s \in S}
    ...
    \right \Vert^2.
\end{gather*}
Таким образом, вычисляем поворот $R$, переформулируя задачу так, чтобы смещение было равно нулю
\begin{equation*}
  \vec{x}_s = \vec{s} - \overline{s}, \,
  \vec{y}_s = \vec{k}_s - \overline{k}_s.
\end{equation*}
Ищем оптимальный поворот $R$ такой, что
\begin{equation}\label{eq:R}
  R =
  \arg \min \limits_{R \in SO \left( 3 \right) }
    \sum \limits_{s \in S} \left \Vert R \cdot \vec{x}_s - \vec{y}_s \right \Vert^2.
\end{equation}
Упростим выражение, которое пытаемся минимизировать в \eqref{eq:R}
\begin{gather*}
  ...
  \vec{x}_s^T \cdot R^T \cdot \vec{y}_s + \vec{y}_s^T \cdot \vec{y}_s.
\end{gather*}
Использовали то, что для матрицы поворота $R^T \cdot R = I$ --- единичная матрица.
...
То же самое справедливо для константы, на которую, поэтому
\begin{equation*}
   \arg \min \limits_{R \in SO \left( 3 \right) }
    \left( -2 \sum \limits_{s \in S} \vec{y}_s^T \cdot R \cdot \vec{x}_s \right) =
   \arg \max \limits_{R \in SO \left( 3 \right) }
    \sum \limits_{R \in SO \left( 3 \right) } \vec{y}_s^T \cdot R \cdot \vec{x}_s.
\end{equation*}
Заметим, что
\begin{equation*}
...
Ищем такой поворот,
который будет давать максимум выражению $tr \left( Y^T \cdot R \cdot X \right) $.
След матрицы имеет свойство
\begin{equation*}
  tr \left( A \cdot B \right) =
  tr \left( B \cdot A \right)
\end{equation*}
для любых матриц $A, \, B$ совместимых размерностей.
Поэтому
\begin{equation*}
  tr \left( Y^T \cdot R \cdot X \right) =
...
Глобальным минимумом был бы такой поворот и смещение,
когда все их соответствующие точки имели бы одинаковое положение.
