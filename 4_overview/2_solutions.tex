...
Возьмём сингулярное разложение \cite{golub:svd} матрицы $S$
\begin{equation*}
...
Если матрица $ \Sigma $ имеет нулевые элементы на диагонали,
то это никак не влияет на результат.
Все элементы матрицы $M$ ограничиваются единицами, поэтому для тех $m_{ii}$,
где $ \sigma_i = 0$, берём максимальное значение, то есть $m_{ii} = 1$.
Снова получаем единичную матрицу.
...
Это эквивалентно тому, что $detM = -1$.
...
Алгоритм состоит в поочерёдном выполнении двух шагог:
\begin{enumerate}
  \item поиск наилучшей разметки $k$ при фикчированных $R$ и $b$;
  \item поиск матрицы поворота $R$ и вектора сдвига $ \vec{b}$ при фиксированной $k$,
\end{enumerate}
пока не будет достигнут минимум в \eqref{eq:least:squares}.
Таким образом, имеем покоординатный спуск, который не гарантирует достижение минимума.
Метод имеет ограниченное применение и хорошо работает только в том случае,
когда нет шума, то есть $ \vec{ \xi }_s = 0$ для всех точек $ \vec{s} \in S$,
а также когда угол поворота и сдвиг можества $S$ относительно множества $T$ малы.
...
Множество $T$ изображено пунктиром, а множество $S$ --- сплошной линией.
