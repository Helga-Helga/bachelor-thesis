...
Такое возможно, если:
\begin{itemize}
  \item матрица $ \tilde{S}$ или матрица $ \tilde{K}^T$
  имеет меньше трёх линейно независимых векторов;
  \item $ \det{X} = \det{ \left( \tilde{S} \cdot \tilde{K}^T \right) } = 0$ \cite{faddeev:rank:det}.
\end{itemize}
