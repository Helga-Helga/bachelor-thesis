...
\begin{gather*}
  \sum \limits_{i = 1}^n \tilde{s}_{ix} \cdot \tilde{k}_{s_i x}^T =
  \sum \limits_{i = 1}^n
    \tilde{s}_{ix} \cdot \left[
      R \vec{s}_i + \vec{b} + \vec{ \xi }_{s_i} -
      \frac{1}{n} \sum \limits_{j = 1}^n
        \left( R \cdot \vec{s}_j + \vec{b} + \vec{ \xi }_{s_j} \right) \right]_x^T = \\
  = \sum \limits_{i = 1}^n
    \tilde{s}_{ix} \cdot \left(
    R \cdot \vec{s}_i + \vec{b} - \frac{1}{n} \sum \limits_{j = 1}^n
      \left( R \cdot \vec{s}_j + \vec{b} \right) \right)_x^T +
  \sum \limits_{i = 1}^n
    \tilde{s}_{ix} \cdot \left(
      \vec{ \xi }_{s_i} - \frac{1}{n} \sum \limits_{j = 1}^n \vec{ \xi }_{s_j} \right)_x^T \sim \\
  \sim N \left(
    \sum \limits_{i = 1}^n
      \tilde{s}_{ix} \cdot \left[ R \cdot \vec{s}_i + \vec{b} -
      \frac{1}{n} \sum \limits_{j = 1}^n \left( R \cdot \vec{s}_j + \vec{b} \right) \right]_x^T,
      \frac{n - 1}{n} \cdot \sigma^2 \sum \limits_{i = 1}^n \tilde{s}_{ix}^2
  \right).
\end{gather*}
...
Покажем, что итераивный алгоритм ближайших точек всегда сходится.
